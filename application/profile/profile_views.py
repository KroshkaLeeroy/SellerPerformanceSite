    ...
    if request.method == 'POST':
        ...
        if not date_to or not date_from:
            ...
            try:
                ...
                if not (yesterday.date() <= date_to_obj.date() <= thirty_two_days_ago.date()):
                    flash('date_to должен быть вчерашним днем и не ранее 32-х дней от текущей даты')
                    return redirect(url_for('profile_blueprint.query_page'))
                ...
                flash('Ошибка в формате даты: ожидается формат DD.MM.YYYY')
